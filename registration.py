...
def feature_registration(source, source_depth,target, target_depth, MIN_MATCH_COUNT = 4):
    """
    ...
    """
    cad_src, depth_src = source, source_depth
    cad_des, depth_des = target, target_depth
    ...
    if len(good)>MIN_MATCH_COUNT:
        ...
        print(len(src_index[0]), len(src_index[1]))
        print(dst_index)
        dst_good=[]
        src_good=[]
        dst_depths = dst_depths[0]
        src_depths = src_depths[0]
...
if __name__ == '__main__':
    ...
    print(transform)
    draw_registration_result(pcd1, pcd2, transform, mode='color')
